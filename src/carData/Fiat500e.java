...
public class Fiat500e extends electicCars.Cars implements electicCars.Graphable{

	private ArrayList<Integer> chargeData = new ArrayList<Integer>();
	
	//In USD
	private final Integer cost = 32600;
	
	private final Integer milesPerCharge = 84;
	
	//In minutes
	private final Integer chargeTime = 219;
	...
	public void storeData(Integer powerLeft) {
		...
		try (PrintWriter pw = new PrintWriter(new File("Fiat500e.txt"))) {
	...
	public void loadData() {
		try (Scanner sc = new Scanner(new File("Fiat500e.txt"))) {
			while (true) {
